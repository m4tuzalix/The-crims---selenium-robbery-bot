...
class crims_gang():
    ...
    def __init__(self, login, password, counter=0):
        ...
        self.browser = webdriver.Chrome("C:\projects\selenium\The-crims---selenium-robbery-gang\chromedriver.exe")
            ...
            for i in inside_club:
                try:
                    if float(i.text[:-1]) == max(stamina_number):
                        ...
                        WebDriverWait(self.browser, 20).until(EC.invisibility_of_element_located((By.CLASS_NAME, "overlay"))) #/// waits until covering element is gon
                        self.gang()
